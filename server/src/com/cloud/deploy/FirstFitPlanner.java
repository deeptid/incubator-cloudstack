...
public class FirstFitPlanner extends PlannerBase implements DeploymentPlanner {
    ...
    public DeployDestination plan(
        ...
        if (s_logger.isDebugEnabled()) {
            ...
            s_logger.debug("Trying to allocate a host and storage pools from dc:"
                    ...
                    + plan.getClusterId()
                    + ", requested cpu: "
                    + cpu_requested + ", requested ram: " + ram_requested);

            s_logger.debug("Is ROOT volume READY (pool already allocated)?: "
                    + (plan.getPoolId() != null ? "Yes" : "No"));
        }

        String haVmTag = (String) vmProfile
                .getParameter(VirtualMachineProfile.Param.HaTag);

        if (plan.getHostId() != null && haVmTag == null) {
            Long hostIdSpecified = plan.getHostId();
            if (s_logger.isDebugEnabled()) {
                s_logger.debug("DeploymentPlan has host_id specified, making no checks on this host, looks like admin test: "
                        + hostIdSpecified);
            }
            ...
            DataCenterDeployment lastPlan = new DataCenterDeployment(
                    host.getDataCenterId(), host.getPodId(),
                    host.getClusterId(), hostIdSpecified, plan.getPoolId(),
                    null, plan.getReservationContext());

            Pair<Map<Volume, List<StoragePool>>, List<Volume>> result = findSuitablePoolsForVolumes(
                    vmProfile, lastPlan, avoid, HostAllocator.RETURN_UPTO_ALL);
            Map<Volume, List<StoragePool>> suitableVolumeStoragePools = result
                    .first();
            ...
            if (!suitableVolumeStoragePools.isEmpty()) {
                ...
                // add host to list if suitable
                suitableHosts = addSuitableHostToList(vmProfile, suitableHosts,
                        host);
                ...
                if (potentialResources != null) {
                    ...
                    Map<Volume, StoragePool> storageVolMap = potentialResources
                            .second();
                    // remove the reused vol<->pool from destination, since we
                    // don't have to prepare this volume.
                    for (Volume vol : readyAndReusedVolumes) {
                        storageVolMap.remove(vol);
                    }
                    DeployDestination dest = new DeployDestination(dc, pod,
                            cluster, host, storageVolMap);
        ...
        if (vm.getLastHostId() != null && haVmTag == null) {
            s_logger.debug("This VM has last host_id specified, trying to choose the same host: "
                    + vm.getLastHostId());
            ...
            if (host == null) {
                ...
                if (host.getStatus() == Status.Up
                    ...
                    ClusterDetailsVO cluster_detail_cpu = _clusterDetailsDao
                            .findDetail(cluster_id, "cpuOvercommitRatio");
                    ClusterDetailsVO cluster_detail_ram = _clusterDetailsDao
                            .findDetail(cluster_id, "memoryOvercommitRatio");
                    Float cpuOvercommitRatio = Float
                            .parseFloat(cluster_detail_cpu.getValue());
                    Float memoryOvercommitRatio = Float
                            .parseFloat(cluster_detail_ram.getValue());
                    if (_capacityMgr.checkIfHostHasCapacity(host.getId(),
                        ...
                        DataCenterDeployment lastPlan = new DataCenterDeployment(
                                host.getDataCenterId(), host.getPodId(),
                                host.getClusterId(), host.getId(),
                                plan.getPoolId(), null);
                        Pair<Map<Volume, List<StoragePool>>, List<Volume>> result = findSuitablePoolsForVolumes(
                                vmProfile, lastPlan, avoid,
                        ...
                        if (!suitableVolumeStoragePools.isEmpty()) {
                            List<Host> suitableHosts = new ArrayList<Host>();
                            // add host to list if suitable
                            suitableHosts = addSuitableHostToList(vmProfile,
                                    suitableHosts, host);
                            Pair<Host, Map<Volume, StoragePool>> potentialResources = findPotentialDeploymentResources(
                                    suitableHosts, suitableVolumeStoragePools);
                            if (potentialResources != null) {
                                Pod pod = _podDao.findById(host.getPodId());
                                Cluster cluster = _clusterDao.findById(host
                                        .getClusterId());
                                Map<Volume, StoragePool> storageVolMap = potentialResources
                                        .second();
                                ...
                                DeployDestination dest = new DeployDestination(
                                        dc, pod, cluster, host, storageVolMap);
        ...
        if (plan.getClusterId() != null) {
            ...
            s_logger.debug("Searching resources only under specified Cluster: "
                    + clusterIdSpecified);
            ClusterVO cluster = _clusterDao.findById(plan.getClusterId());
            if (cluster != null) {
                clusterList.add(clusterIdSpecified);
                return checkClustersforDestination(clusterList, vmProfile,
                        plan, avoid, dc);
            } else {
            ...
            s_logger.debug("Searching resources only under specified Pod: "
                    + podIdSpecified);

            HostPodVO pod = _podDao.findById(podIdSpecified);
            if (pod != null) {
                DeployDestination dest = scanClustersForDestinationInZoneOrPod(
                        podIdSpecified, false, vmProfile, plan, avoid);
                if (dest == null) {
            ...
            s_logger.debug("Searching all possible resources under this Zone: "
                    + plan.getDataCenterId());

            boolean applyAllocationAtPods = Boolean.parseBoolean(_configDao
                    .getValue(Config.ApplyAllocationAlgorithmToPods.key()));
            if (applyAllocationAtPods) {
                ...
                return scanClustersForDestinationInZoneOrPod(
                        plan.getDataCenterId(), true, vmProfile, plan, avoid);
            }
        }

    }

    private DeployDestination scanPodsForDestination(
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            DeploymentPlan plan, ExcludeList avoid) {
        ...
        Pair<List<Long>, Map<Long, Double>> podCapacityInfo = listPodsByCapacity(
                plan.getDataCenterId(), requiredCpu, requiredRam);
        List<Long> podsWithCapacity = podCapacityInfo.first();

        if (!podsWithCapacity.isEmpty()) {
            if (avoid.getPodsToAvoid() != null) {
                if (s_logger.isDebugEnabled()) {
                    s_logger.debug("Removing from the podId list these pods from avoid set: "
                            + avoid.getPodsToAvoid());
                }
                podsWithCapacity.removeAll(avoid.getPodsToAvoid());
            }
            if (!isRootAdmin(plan.getReservationContext())) {
                List<Long> disabledPods = listDisabledPods(plan
                        .getDataCenterId());
                if (!disabledPods.isEmpty()) {
                    if (s_logger.isDebugEnabled()) {
                        s_logger.debug("Removing from the podId list these pods that are disabled: "
                                + disabledPods);
                    }
        ...
        if (!podsWithCapacity.isEmpty()) {
            ...
            for (Long podId : prioritizedPodIds) {
                ...
                DeployDestination dest = scanClustersForDestinationInZoneOrPod(
                        podId, false, vmProfile, plan, avoid);
                if (dest != null) {
                    return dest;
                }
                avoid.addPod(podId);
    ...
    private DeployDestination scanClustersForDestinationInZoneOrPod(long id,
            boolean isZone,
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            DeploymentPlan plan, ExcludeList avoid) {
        ...
        Pair<List<Long>, Map<Long, Double>> clusterCapacityInfo = listClustersByCapacity(
                id, requiredCpu, requiredRam, avoid, isZone);
        List<Long> prioritizedClusterIds = clusterCapacityInfo.first();
        if (!prioritizedClusterIds.isEmpty()) {
            if (avoid.getClustersToAvoid() != null) {
                if (s_logger.isDebugEnabled()) {
                    s_logger.debug("Removing from the clusterId list these clusters from avoid set: "
                            + avoid.getClustersToAvoid());
                }
            ...
            if (!isRootAdmin(plan.getReservationContext())) {
                ...
                if (isZone) {
                    disabledClusters = listDisabledClusters(
                            plan.getDataCenterId(), null);
                } else {
                    disabledClusters = listDisabledClusters(
                            plan.getDataCenterId(), id);
                }
                if (!disabledClusters.isEmpty()) {
                    if (s_logger.isDebugEnabled()) {
                        s_logger.debug("Removing from the clusterId list these clusters that are disabled/clusters under disabled pods: "
                                + disabledClusters);
                    }
        ...
        if (!prioritizedClusterIds.isEmpty()) {
            List<Long> clusterList = reorderClusters(id, isZone,
                    clusterCapacityInfo, vmProfile, plan);
            return checkClustersforDestination(clusterList, vmProfile, plan,
                    avoid, dc);
        } else {
            if (s_logger.isDebugEnabled()) {
    ...
    private List<Host> addSuitableHostToList(
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            List<Host> suitableHosts, HostVO host) {
        // check for dedicated resources
        VirtualMachine vm = vmProfile.getVirtualMachine();
        ServiceOffering offering = vmProfile.getServiceOffering();
        long hostId = host.getId();
        DedicatedResourceVO dedicatedHost = _dedicatedDao.findByHostId(hostId);
        DedicatedResourceVO dedicatedClusterOfHost = _dedicatedDao
                .findByClusterId(host.getClusterId());
        DedicatedResourceVO dedicatedPodOfHost = _dedicatedDao.findByPodId(host
                .getPodId());
        AccountVO account = _accountDao.findById(vm.getAccountId());
        if (!offering.getImplicitDedication() && !vm.getUseDedication()) {
            if (dedicatedHost == null && dedicatedClusterOfHost == null
                    && dedicatedPodOfHost == null) {
                suitableHosts.add(host);
            } else if (offering.getImplicitDedication()) {
                if (checkForImplicitDedication(host, account)) {
                    suitableHosts.add(host);
                }
            } else if (vm.getUseDedication()) {
        ...
        if (checkIfHostIsDedicated(host)) {
            ...
            if (!implicit) {
                ...
                if (accountIdOfDedicatedHost == null) {
                    // check if host is dedicated to a domain
                    if (dedicatedHost.getDomainId() != null
                            && dedicatedHost.getDomainId() == account
                                    .getDomainId()) {
                        suitable = true;
                    }
                } else {
                    if (dedicatedHost.getAccountId() == account.getId()
                            || dedicatedClusterOfHost.getAccountId() == account
                                    .getId()
                            || dedicatedPodOfHost.getAccountId() == account
                                    .getId()) {
                        suitable = true;
                    }
                }
    ...
    private boolean checkForImplicitDedication(HostVO host, Account account) {
        ...
        if (checkIfHostIsDedicated(host)) {
            boolean implicit = dedicatedHost.getImplicitDedication();
            boolean implicitCluster = dedicatedClusterOfHost
                    .getImplicitDedication();
            boolean implicitPod = dedicatedPodOfHost.getImplicitDedication();
            if (implicit || implicitCluster || implicitPod) {
                boolean dedicatedVms = _vmInstanceDao
                        .checkIfHosthasDedicatedVms(hostId, account.getId());
     ...
     * necessary heuristic for this planner For FirstFitPlanner there is no
     * specific heuristic to be applied other than the capacity based ordering
     ...
     */
    protected List<Long> reorderPods(
            Pair<List<Long>, Map<Long, Double>> podCapacityInfo,
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            DeploymentPlan plan) {
    ...
    private List<Long> listDisabledClusters(long zoneId, Long podId) {
        List<Long> disabledClusters = _clusterDao.listDisabledClusters(zoneId,
                podId);
        if (podId == null) {
            // list all disabled clusters under this zone + clusters under any
            // disabled pod of this zone
            List<Long> clustersWithDisabledPods = _clusterDao
                    .listClustersWithDisabledPods(zoneId);
    ...
    private Map<Short, Float> getCapacityThresholdMap() {
        ...
        String cpuDisableThresholdString = _configDao
                .getValue(Config.CPUCapacityDisableThreshold.key());
        float cpuDisableThreshold = NumbersUtil.parseFloat(
                cpuDisableThresholdString, 0.85F);
        disableThresholdMap
                .put(Capacity.CAPACITY_TYPE_CPU, cpuDisableThreshold);

        String memoryDisableThresholdString = _configDao
                .getValue(Config.MemoryCapacityDisableThreshold.key());
        float memoryDisableThreshold = NumbersUtil.parseFloat(
                memoryDisableThresholdString, 0.85F);
    ...
    private void removeClustersCrossingThreshold(
            List<Long> clusterListForVmAllocation, ExcludeList avoid,
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            DeploymentPlan plan) {
        ...
        for (short capacity : capacityList) {
            ...
            if (capacity == Capacity.CAPACITY_TYPE_CPU) {
                clustersCrossingThreshold = _capacityDao
                        .listClustersCrossingThreshold(capacity,
                                plan.getDataCenterId(),
            ...
            if (clustersCrossingThreshold != null
                ...
                s_logger.debug("Cannot allocate cluster list "
                        ...
                        + " crosses the disable capacity threshold: "
                        + capacityThresholdMap.get(capacity)
                        + " for capacity Type : " + capacity
                        + ", skipping these clusters");
            }
    ...
    private DeployDestination checkClustersforDestination(
        ...
        for (Long clusterId : clusterList) {
            ...
            DataCenterDeployment potentialPlan = new DataCenterDeployment(
                    plan.getDataCenterId(), clusterVO.getPodId(),
            ...
            if (suitableHosts != null && !suitableHosts.isEmpty()) {
                ...
                if (!suitableVolumeStoragePools.isEmpty()) {
                    ...
                    if (potentialResources != null) {
                        Pod pod = _podDao.findById(clusterVO.getPodId());
                        Host host = _hostDao.findById(potentialResources
                                .first().getId());
                        Map<Volume, StoragePool> storageVolMap = potentialResources
                                .second();
                        ...
                        DeployDestination dest = new DeployDestination(dc, pod,
                                clusterVO, host, storageVolMap);
                        s_logger.debug("Returning Deployment Destination: "
                                + dest);
                        return dest;
                    }
                } else {
                    s_logger.debug("No suitable storagePools found under this Cluster: "
                            + clusterId);
                }
            } else {
    ...
    protected Pair<List<Long>, Map<Long, Double>> listClustersByCapacity(
            long id, int requiredCpu, long requiredRam, ExcludeList avoid,
            boolean isZone) {
        // look at the aggregate available cpu and ram per cluster
        // although an aggregate value may be false indicator that a cluster can
        // host a vm, it will at the least eliminate those clusters which
        // definitely cannot

        // we need clusters having enough cpu AND RAM to host this particular VM
        // and order them by aggregate cluster capacity
        if (s_logger.isDebugEnabled()) {
        ...
        String capacityTypeToOrder = _configDao
                .getValue(Config.HostCapacityTypeToOrderClusters.key());
        short capacityType = CapacityVO.CAPACITY_TYPE_CPU;
        if ("RAM".equalsIgnoreCase(capacityTypeToOrder)) {
            capacityType = CapacityVO.CAPACITY_TYPE_MEMORY;
        }

        List<Long> clusterIdswithEnoughCapacity = _capacityDao
                .listClustersInZoneOrPodByHostCapacities(id, requiredCpu,
                        requiredRam, capacityType, isZone);
        if (s_logger.isTraceEnabled()) {
            s_logger.trace("ClusterId List having enough CPU and RAM capacity: "
                    + clusterIdswithEnoughCapacity);
        }
        Pair<List<Long>, Map<Long, Double>> result = _capacityDao
                .orderClustersByAggregateCapacity(id, capacityType, isZone);
        ...
        if (s_logger.isTraceEnabled()) {
            s_logger.trace("ClusterId List in order of aggregate capacity: "
                    + clusterIdsOrderedByAggregateCapacity);
        }
        clusterIdsOrderedByAggregateCapacity
                .retainAll(clusterIdswithEnoughCapacity);

        if (s_logger.isTraceEnabled()) {
            s_logger.trace("ClusterId List having enough CPU and RAM capacity & in order of aggregate capacity: "
                    + clusterIdsOrderedByAggregateCapacity);
        }
    ...
    protected Pair<List<Long>, Map<Long, Double>> listPodsByCapacity(
            long zoneId, int requiredCpu, long requiredRam) {
        // look at the aggregate available cpu and ram per pod
        // although an aggregate value may be false indicator that a pod can
        // host a vm, it will at the least eliminate those pods which definitely
        // cannot

        // we need pods having enough cpu AND RAM to host this particular VM and
        // order them by aggregate pod capacity
        if (s_logger.isDebugEnabled()) {
            s_logger.debug("Listing pods in order of aggregate capacity, that have (atleast one host with) enough CPU and RAM capacity under this Zone: "
                    + zoneId);
        }
        String capacityTypeToOrder = _configDao
                .getValue(Config.HostCapacityTypeToOrderClusters.key());
        ...
        List<Long> podIdswithEnoughCapacity = _capacityDao
                .listPodsByHostCapacities(zoneId, requiredCpu, requiredRam,
                        capacityType);
        if (s_logger.isTraceEnabled()) {
            s_logger.trace("PodId List having enough CPU and RAM capacity: "
                    + podIdswithEnoughCapacity);
        }
        Pair<List<Long>, Map<Long, Double>> result = _capacityDao
                .orderPodsByAggregateCapacity(zoneId, capacityType);
        List<Long> podIdsOrderedByAggregateCapacity = result.first();
        // only keep the clusters that have enough capacity to host this VM
        if (s_logger.isTraceEnabled()) {
            s_logger.trace("PodId List in order of aggregate capacity: "
                    + podIdsOrderedByAggregateCapacity);
        }
        podIdsOrderedByAggregateCapacity.retainAll(podIdswithEnoughCapacity);

        if (s_logger.isTraceEnabled()) {
            s_logger.trace("PodId List having enough CPU and RAM capacity & in order of aggregate capacity: "
                    + podIdsOrderedByAggregateCapacity);
        }
    ...
    protected Pair<Host, Map<Volume, StoragePool>> findPotentialDeploymentResources(
        ...
        TreeSet<Volume> volumesOrderBySizeDesc = new TreeSet<Volume>(
        ...
        for (Host potentialHost : suitableHosts) {
            ...
            for (Volume vol : volumesOrderBySizeDesc) {
                ...
                s_logger.debug("Checking if host: " + potentialHost.getId()
                        ...
                        + vol.getVolumeType());
                List<StoragePool> volumePoolList = suitableVolumeStoragePools
                        .get(vol);
                    ...
                    if (hostCanAccessSPool(potentialHost, potentialSPool)) {
                        ...
                        if (multipleVolume) {
                            ...
                            if (volumeAllocationMap.containsKey(potentialSPool))
                                requestVolumes = volumeAllocationMap
                                        .get(potentialSPool);
                            else
                                requestVolumes = new ArrayList<Volume>();
                            requestVolumes.add(vol);

                            if (!_storageMgr.storagePoolHasEnoughSpace(
                                    requestVolumes, potentialSPool))
                                continue;
    ...
    protected boolean hostCanAccessSPool(Host host, StoragePool pool) {
        ...
        StoragePoolHostVO hostPoolLinkage = _poolHostDao.findByPoolHost(
                pool.getId(), host.getId());
        if (hostPoolLinkage != null) {
            hostCanAccessSPool = true;
        }

        s_logger.debug("Host: " + host.getId()
                + (hostCanAccessSPool ? " can" : " cannot") + " access pool: "
                + pool.getId());
    ...
    protected Pair<Map<Volume, List<StoragePool>>, List<Volume>> findSuitablePoolsForVolumes(
            VirtualMachineProfile<? extends VirtualMachine> vmProfile,
            DeploymentPlan plan, ExcludeList avoid, int returnUpTo) {
        List<VolumeVO> volumesTobeCreated = _volsDao
                .findUsableVolumesForInstance(vmProfile.getId());
        ...
        for (VolumeVO toBeCreated : volumesTobeCreated) {
            ...
            if (plan.getPoolId() != null) {
                ...
                if (toBeCreated.getPoolId() != null) {
                    pool = (StoragePool) this.dataStoreMgr
                            .getPrimaryDataStore(toBeCreated.getPoolId());
                } else {
                    pool = (StoragePool) this.dataStoreMgr
                            .getPrimaryDataStore(plan.getPoolId());
                }
                ...
                if (!pool.isInMaintenance()) {
                    if (!avoid.shouldAvoid(pool)) {
                        ...
                        long exstPoolPodId = pool.getPodId() != null ? pool
                                .getPodId() : -1;
                        long exstPoolClusterId = pool.getClusterId() != null ? pool
                                .getClusterId() : -1;
                        if (plan.getDataCenterId() == exstPoolDcId
                            ...
                            suitableVolumeStoragePools.put(toBeCreated,
                                    suitablePools);
                            if (!(toBeCreated.getState() == Volume.State.Allocated || toBeCreated
                                    .getState() == Volume.State.Creating)) {
            ...
            DiskOfferingVO diskOffering = _diskOfferingDao.findById(toBeCreated
                    .getDiskOfferingId());
            ...
            if (vmProfile.getType() != VirtualMachine.Type.User) {
                String ssvmUseLocalStorage = _configDao
                        .getValue(Config.SystemVMUseLocalStorage.key());
            ...
            for (StoragePoolAllocator allocator : _storagePoolAllocators) {
                final List<StoragePool> suitablePools = allocator
                        .allocateToPool(diskProfile, vmProfile, plan, avoid,
                                returnUpTo);
        ...
        return new Pair<Map<Volume, List<StoragePool>>, List<Volume>>(
                suitableVolumeStoragePools, readyAndReusedVolumes);
    }

    private boolean isRootAdmin(ReservationContext reservationContext) {
        if (reservationContext != null) {
            if (reservationContext.getAccount() != null) {
                return _accountMgr.isRootAdmin(reservationContext.getAccount()
                        .getType());
            } else {
                return false;
            }
    ...
    @Override
    public boolean check(VirtualMachineProfile<? extends VirtualMachine> vm,
            DeploymentPlan plan, DeployDestination dest, ExcludeList exclude) {
    ...
    public boolean canHandle(
        ...
        if (vm.getHypervisorType() != HypervisorType.BareMetal) {
            ...
            if (_allocationAlgorithm != null
                    && (_allocationAlgorithm.equals(AllocationAlgorithm.random
                            .toString()) || _allocationAlgorithm
                            .equals(AllocationAlgorithm.firstfit.toString()))
                    && !offering.getImplicitDedication()
                    && !virtualMachine.getUseDedication()) {
                return true;
            }
        }
        return false;
    ...
    public boolean configure(String name, Map<String, Object> params)
        ...
        _allocationAlgorithm = _configDao.getValue(Config.VmAllocationAlgorithm
                .key());
        return true;
    }

    private boolean isEnabledForAllocation(long zoneId, Long podId,
            Long clusterId) {
        // Check if the zone exists in the system
        DataCenterVO zone = _dcDao.findById(zoneId);
        if (zone != null
                && Grouping.AllocationState.Disabled == zone
                        .getAllocationState()) {
            s_logger.info("Zone is currently disabled, cannot allocate to this zone: "
                    + zoneId);
            return false;
        }
        ...
        if (pod != null
                && Grouping.AllocationState.Disabled == pod
                        .getAllocationState()) {
            s_logger.info("Pod is currently disabled, cannot allocate to this pod: "
                    + podId);
            return false;
        }
        ...
        if (cluster != null
                && Grouping.AllocationState.Disabled == cluster
                        .getAllocationState()) {
            s_logger.info("Cluster is currently disabled, cannot allocate to this cluster: "
                    + clusterId);
            return false;
        }
