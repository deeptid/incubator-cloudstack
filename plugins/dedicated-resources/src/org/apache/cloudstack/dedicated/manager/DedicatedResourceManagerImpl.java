...
public class DedicatedResourceManagerImpl implements DedicatedService {
    ...
    public List<DedicatedResourceVO> dedicateZone(Long zoneId, Long domainId, Long accountId, String accountName) {
        if(accountName != null){
        ...
        if (dc == null) {
            ...
            boolean releasePod = true;
            boolean releaseCluster = true;
            boolean releaseHost = true;
            for (HostPodVO pod : pods) {
                DedicatedResourceVO dPod = _dedicatedDao.findByPodId(pod.getId());
                if (dPod != null) {
                    if(!(getDomainChildIds(domainId).contains(dPod.getDomainId()))) {
                        throw new CloudRuntimeException("Pod " + pod.getName() + " under this Zone " + dc.getName() + " is dedicated to different account/domain");
                    }
                    if (dPod != null && dPod.getAccountId() == accountId && dPod.getDomainId() == domainId) {
            ...
            for (ClusterVO cluster : clusters) {
                ...
                if (dCluster != null) {
                    if(!(getDomainChildIds(domainId).contains(dCluster.getDomainId()))) {
                    ...
                    if (dCluster.getAccountId() == accountId && dCluster.getDomainId() == domainId) {
            ...
            for (HostVO host : hosts) {
                ...
                if (dHost != null) {
                    ...
                    if (dHost.getAccountId() == accountId && dHost.getDomainId() == domainId) {
    ...
    public List<DedicatedResourceVO> dedicatePod(Long podId, Long domainId, Long accountId, String accountName) {
        if(accountName != null){
            Account caller = UserContext.current().getCaller();
            Account owner = _accountMgr.finalizeOwner(caller, accountName, domainId, null);
            accountId = owner.getId();
    ...
    public List<DedicatedResourceVO> dedicateCluster(Long clusterId, Long domainId, Long accountId, String accountName) {
        if(accountName != null){
            Account caller = UserContext.current().getCaller();
            Account owner = _accountMgr.finalizeOwner(caller, accountName, domainId, null);
            accountId = owner.getId();
    ...
    public List<DedicatedResourceVO> dedicateHost(Long hostId, Long domainId, Long accountId, String accountName) {
        if(accountName != null){
            Account caller = UserContext.current().getCaller();
            Account owner = _accountMgr.finalizeOwner(caller, accountName, domainId, null);
            accountId = owner.getId();
