...
@APICommand(name = "listDedicatedHosts", description="Lists dedicated hosts.", responseObject=ClusterResponse.class)
public class ListDedicatedHostsCmd extends BaseListCmd {
    ...
    @Override
    public void execute(){
    	List<? extends DedicatedResources> result = _dedicatedservice.listDedicatedHosts(this);
        ...
        if (result != null) {
        	 for (DedicatedResources resource : result) {
        		 DedicateHostResponse hostResponse = _dedicatedservice.createDedicateHostResponse(resource);
